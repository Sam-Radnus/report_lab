...
PORTFOLIO = {
    # Original stocks (rescaled percentages for balance)
    'AAPL': 91,
    'MSFT': 123,
    'GOOGL': 14,
    'TSLA': 71,
...
def calculate_advanced_metrics(portfolio_values, benchmark_data):
    ...
    if benchmark_data is not None and len(benchmark_data) > 0:
        # Get benchmark returns for the same period as portfolio
...
def create_pie_chart(portfolio_data, width=3.5, height=3.5):
    """Generate pie chart as image buffer"""
...
def create_line_chart(portfolio_history, width=6, height=3):
    """Generate line chart as image buffer"""
    fig, ax = plt.subplots(figsize=(width, height))
...
def create_pdf_dashboard(portfolio_data, total_value, total_cost, portfolio_history, metrics, filename):
    ...
    for holding in portfolio_data:
        change_color = 'green' if holding['day_change'] > 0 else 'red' if holding['day_change'] < 0 else 'black'
        table_data.append([
            holding['ticker'],
            f"{holding['shares']:.0f}",
...
def main():
    ...
    output_file = f"portfolio_dashboard_{datetime.now().strftime('%Y%m%d')}.pdf"
    create_pdf_dashboard(portfolio_data, total_value, total_cost, portfolio_history, metrics, output_file)
